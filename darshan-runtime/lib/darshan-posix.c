...
#define POSIX_RECORD_OPEN(__ret, __path, __mode, __tm1, __tm2, __tv1, __tv2) do { \
    ...
    printf("we are in the regular POSIX_RECORD_OPEN \n");\
     if(getenv("DXT_ENABLE_LDMS") || getenv("POSIX_ENABLE_LDMS")){\
        ...
        darshan_ldms_connector_send(__rec_ref->file_rec->counters[POSIX_OPENS], "open", -1, -1, -1, -1, -1, __tm1, __tm2, __tv1, __tv2, __rec_ref->file_rec->fcounters[POSIX_F_META_TIME], "POSIX", "MET");\
     }\
} while(0)

#define POSIX_RECORD_REFOPEN(__ret, __rec_ref, __tm1, __tm2, __tv1, __tv2, __ref_counter) do { \
    printf("we are in the posix_record_refopen \n");\
    if(__ret < 0 || !__rec_ref) break; \
    _POSIX_RECORD_OPEN(__ret, __rec_ref, 0, __tm1, __tm2, __tv1, __tv2,  0, __ref_counter); \
} while(0)

#define _POSIX_RECORD_OPEN(__ret, __rec_ref, __mode, __tm1, __tm2, __tv1, __tv2, __reset_flag, __ref_counter) do { \
    if(__mode) __rec_ref->file_rec->counters[POSIX_MODE] = __mode; \
    printf("we are in the _POSIX_RECORD_OPEN \n");\
    if(__reset_flag) { \
        __rec_ref->offset = 0; \
...
#define POSIX_RECORD_READ(__ret, __fd, __pread_flag, __pread_offset, __aligned, __tm1, __tm2, __tv1, __tv2) do { \
    ...
    dxt_posix_read(rec_ref->file_rec->base_rec.id, this_offset, __ret, __tm1, __tm2, __tv1, __tv2); \
    ...
    /* LDMS to publish realtime read tracing information to daemon*/ \
    if(getenv("DXT_ENABLE_LDMS") || getenv("POSIX_ENABLE_LDMS"))\
        darshan_ldms_connector_send(rec_ref->file_rec->counters[POSIX_READS], "read", this_offset, __ret, rec_ref->file_rec->counters[POSIX_MAX_BYTE_READ],rec_ref->file_rec->counters[POSIX_RW_SWITCHES], -1,  __tm1, __tm2, __tv1, __tv2, rec_ref->file_rec->fcounters[POSIX_F_READ_TIME], "POSIX", "MOD");\
} while(0)
...
#define POSIX_RECORD_WRITE(__ret, __fd, __pwrite_flag, __pwrite_offset, __aligned, __tm1, __tm2, __tv1, __tv2) do { \
    ...
    if(__pwrite_flag) \
        ...
        this_offset = rec_ref->offset; \
    /* DXT to record detailed write tracing information */ \
    dxt_posix_write(rec_ref->file_rec->base_rec.id, this_offset, __ret, __tm1, __tm2, __tv1, __tv2); \
    ...
    if(getenv("DXT_ENABLE_LDMS") || getenv("POSIX_ENABLE_LDMS"))\
    darshan_ldms_connector_send(rec_ref->file_rec->counters[POSIX_WRITES], "write", this_offset, __ret, rec_ref->file_rec->counters[POSIX_MAX_BYTE_WRITTEN], rec_ref->file_rec->counters[POSIX_RW_SWITCHES], -1, __tm1, __tm2, __tv1, __tv2, rec_ref->file_rec->fcounters[POSIX_F_WRITE_TIME], "POSIX", "MOD");\
} while(0)
...
#define POSIX_LOOKUP_RECORD_STAT(__path, __statbuf, __tm1, __tm2, __tv1, __tv2) do { \
    ...
    if(newpath != __path) free(newpath); \
    if(rec_ref) { \
        POSIX_RECORD_STAT(rec_ref, __statbuf, __tm1, __tm2, __tv1, __tv2); \
    } \
} while(0)

#define POSIX_RECORD_STAT(__rec_ref, __statbuf, __tm1, __tm2, __tv1, __tv2) do { \
...
int DARSHAN_DECL(open)(const char *path, int flags, ...)
{
    ...
    POSIX_PRE_RECORD();
    POSIX_RECORD_OPEN(ret, path, mode, tm1, tm2, tv1, tv2);
    POSIX_POST_RECORD();
...
int DARSHAN_DECL(__open_2)(const char *path, int oflag)
{
    ...
    POSIX_PRE_RECORD();
    POSIX_RECORD_OPEN(ret, path, 0, tm1, tm2, tv1, tv2);
    POSIX_POST_RECORD();
...
int DARSHAN_DECL(openat)(int dirfd, const char *pathname, int flags, ...)
{
    ...
    if(pathname[0] == '/' || dirfd == AT_FDCWD)
    {
         ...
         *    - dirfd equal to CWD
         */
        POSIX_RECORD_OPEN(ret, pathname, mode, tm1, tm2, tv1, tv2);
    }
    else
    {
        ...
        if(dirpath)
        {
            ...
            POSIX_RECORD_OPEN(ret, tmp_path, mode, tm1, tm2, tv1, tv2);
        }
        else
        {
...
int DARSHAN_DECL(dup)(int oldfd)
{
    ...
    if(ret >= 0)
    {
        ...
        rec_ref = darshan_lookup_record_ref(posix_runtime->fd_hash,
            &oldfd, sizeof(oldfd));
        POSIX_RECORD_REFOPEN(ret, rec_ref, tm1, tm2, tv1, tv2, POSIX_DUPS);
        POSIX_POST_RECORD();
    }
...
int DARSHAN_DECL(fileno)(FILE *stream)
{
    ...
    if(ret >= 0)
    {
        ...
        if(rec_name)
        {
            ...
            POSIX_RECORD_REFOPEN(ret, rec_ref, tm1, tm2, tv1, tv2, POSIX_FILENOS);
            POSIX_POST_RECORD();
        }
    }
...
int DARSHAN_DECL(mkstemp)(char* template)
{
    ...
    POSIX_PRE_RECORD();
    POSIX_RECORD_OPEN(ret, template, 0, tm1, tm2, tv1, tv2);
    POSIX_POST_RECORD();
...
ssize_t DARSHAN_DECL(read)(int fd, void *buf, size_t count)
{
    ...
    POSIX_PRE_RECORD();
    POSIX_RECORD_READ(ret, fd, 0, 0, aligned_flag, tm1, tm2, tv1, tv2);
    POSIX_POST_RECORD();
...
ssize_t DARSHAN_DECL(write)(int fd, const void *buf, size_t count)
{
    ...
    POSIX_PRE_RECORD();
    POSIX_RECORD_WRITE(ret, fd, 0, 0, aligned_flag, tm1, tm2, tv1, tv2);
    POSIX_POST_RECORD();
...
ssize_t DARSHAN_DECL(pread)(int fd, void *buf, size_t count, off_t offset)
{
    ...
    POSIX_PRE_RECORD();
    POSIX_RECORD_READ(ret, fd, 1, offset, aligned_flag, tm1, tm2, tv1, tv2);
    POSIX_POST_RECORD();
...
ssize_t DARSHAN_DECL(pwrite)(int fd, const void *buf, size_t count, off_t offset)
{
    ...
    POSIX_PRE_RECORD();
    POSIX_RECORD_WRITE(ret, fd, 1, offset, aligned_flag, tm1, tm2, tv1, tv2);
    POSIX_POST_RECORD();
...
int DARSHAN_DECL(__xstat)(int vers, const char *path, struct stat *buf)
{
    ...
    POSIX_PRE_RECORD();
    POSIX_LOOKUP_RECORD_STAT(path, buf, tm1, tm2, tv1, tv2);
    POSIX_POST_RECORD();
...
int DARSHAN_DECL(__fxstat)(int vers, int fd, struct stat *buf)
{
    ...
    if(rec_ref)
    {
        POSIX_RECORD_STAT(rec_ref, buf, tm1, tm2, tv1, tv2);
    }
    POSIX_POST_RECORD();
...
int DARSHAN_DECL(close)(int fd)
{
    ...
    if(getenv("DXT_ENABLE_LDMS") || getenv("POSIX_ENABLE_LDMS"))
        darshan_ldms_connector_send(-1, "close", -1, -1, -1, -1, -1, tm1, tm2, tv1, tv2, rec_ref->file_rec->fcounters[POSIX_F_META_TIME], "POSIX", "MET");
#endif
...
static void posix_aio_tracker_add(int fd, void *aiocbp)
{
    ...
    if(rec_ref)
    {
        ...
        if(tracker)
        {
            tracker->tm1 = darshan_core_wtime(&tv1);
...
void darshan_posix_shutdown_bench_setup(int test_case)
{
    ...
    switch(test_case)
    {
        case 1: /* single file-per-process */
            ...
            POSIX_RECORD_OPEN(fd_array[0], filepath, 777, 0, 1, tv1, tv2);
            POSIX_RECORD_WRITE(size_array[0], fd_array[0], 0, 0, 1, 1, 2, tv1, tv2);
        ...
        case 3: /* 1024 unique files per proc */
            for(i = 0; i < 1024; i++)
            {
                ...
                POSIX_RECORD_OPEN(fd_array[i], filepath, 777, 0, 1, tv1, tv2);
                POSIX_RECORD_WRITE(size_array[i % DARSHAN_COMMON_VAL_MAX_RUNTIME_COUNT],
                    fd_array[i], 0, 0, 1, 1, 2, tv1, tv2);
            }
