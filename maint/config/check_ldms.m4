...
AC_ARG_WITH(dxt-ldms,
[  --with-dxt-ldms=DIR root directory path of ldms installation [defaults to
                    /usr/local or /usr if not found in /usr/local]
  --with-dxt-ldms to disable ldms usage completely],
...
AS_IF([test "x$ldms_found_stream_headers" = "xyes"],
	[AC_DEFINE([HAVE_DXT_LDMS],[1], [Define if standard LDMS library headers exist]) AM_CONDITIONAL([HAVE_DXT_LDMS],[test "x$ldms_found_stream_headers" = "xyes"])], 
        [AC_MSG_ERROR([Unable to find the standard LDMS headers for DXT])])
])
